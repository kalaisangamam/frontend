...
const LeadershipCard = ({ master, index }) => (
  <motion.article
    initial={{ opacity: 0, y: 20 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true }}
    transition={{ duration: 0.45, delay: index * 0.06 }}
    className="master-card master-card--leadership group"
  >
...
const ProgrammeCard = ({ master, index }) => (
  <motion.article
  ...
  >
    <p className="text-[0.65rem] font-bold uppercase tracking-[0.16em] text-brass-400">
      {master.programme || "Programme Coach"}
    </p>
...
const SpecialtyFilter = ({ specialties, active, onChange }) => (
  <div
    className="-mx-4 mb-8 overflow-x-auto px-4 pb-1 sm:mx-0 sm:px-0"
    role="group"
    aria-label="Filter coaches by specialty"
  >
...
const Masters = ({ limit, showViewAll = false, pageView = false }) => {
  ...
  const { leadership, programmeMasters, specialties } = useMemo(() => {
    const ordered = [...(masters || [])].sort(
      (a, b) => Number(a.display_order) - Number(b.display_order),
    );
    const programmeMasters = ordered.filter(
      (master) => master.master_type === "programme",
    );
    return {
      leadership: ordered.filter(
      ...
      specialties: [
        ...new Set(
          programmeMasters
            .map((master) => master.specialization?.trim())
            .filter(Boolean),
        ),
      ],
    };
  }, [masters]);
  const visibleProgrammeMasters =
    activeSpecialty === "All"
      ? programmeMasters
      : programmeMasters.filter(
          (master) => master.specialization?.trim() === activeSpecialty,
        );
  const isLoading = !masters && !error;
  const previewLeadership = leadership.slice(0, limit || leadership.length);
  const previewProgramme = programmeMasters.slice(
    0,
    limit || programmeMasters.length,
  );
  const renderGroups = (isFullPage) => (
    <div className={isFullPage ? "space-y-16" : "space-y-10"}>
      {leadership.length > 0 && (
        <section aria-labelledby="leadership-heading">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-7">
            {(isFullPage ? leadership : previewLeadership).map(
              (master, index) => (
                <LeadershipCard key={master.id} master={master} index={index} />
              ),
            )}
          </div>
        </section>
      ...
      {programmeMasters.length > 0 && (
        <section aria-labelledby="programme-masters-heading">
          ...
          >
            {(isFullPage ? visibleProgrammeMasters : previewProgramme).map(
              (master, index) => (
                <ProgrammeCard key={master.id} master={master} index={index} />
              ),
            )}
          </motion.div>
          {isFullPage && visibleProgrammeMasters.length === 0 && (
            <EmptyState message="No coaches available" />
          )}
        </section>
      )}
    </div>
        ...
        <div className="container-xl">
          <header className="max-w-3xl mx-auto text-center mb-16">
            ...
            <h1 className="section-heading">
              Trained under dedicated, experienced coaches
            </h1>
            <p className="mt-4 text-slate-300 text-sm sm:text-base leading-relaxed">
              Our leadership and programme masters bring expertise, discipline
          ...
          {error && (
            <ErrorState message="Couldn't load master profiles right now." />
          )}
          {masters?.length === 0 && (
            <EmptyState message="Master profiles will appear here soon." />
          )}
          {masters?.length > 0 && renderGroups(true)}
        </div>
      </section>
    );
  return (
    <section id="masters" className="py-10">
      <div className="container-xl">
        <SectionHeading
          ...
          subtitle="Meet the leadership and programme coaches guiding every training session."
        />
        {isLoading && (
          <SkeletonGrid
            count={limit || 4}
